class ReminderEmail < ActiveRecord::Base
  ...
  DAYSINC = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
  DAYSDEC = ["Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday", "Monday"]
  ...
  def is_in_last_5mins
    ...
    current_time_reminder = Time.zone.parse(self.string_time) if self.string_time
...
# меняю отображение - start
    ...
    tz       = self.customer.time_zone
    if variable[tz]
      hour_off_set = variable[tz][0]
      mins_off_set = variable[tz][1]
    ...
    hour_from_datetime = self.time.utc.hour
    mins_from_datetime = self.time.utc.min
    ...
    if now_wday > cur_wday
      self.days_of_week.split(',').collect { |index| DAYSINC[Integer(index)] }
    elsif now_wday < cur_wday
      self.days_of_week.split(',').collect { |index| DAYSDEC[Integer(index)] }
# меняю отображение - end
    else

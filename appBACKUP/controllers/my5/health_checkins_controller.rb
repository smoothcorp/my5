class My5::HealthCheckinsController < ApplicationController
  ...
  before_filter :customer_views, :only => [:index, :new]
  ...
  def index
    @health_checkins = health_checkins.where(:status => 1).order("id desc").paginate(:per_page => 20, :page => (params[:page] || 1))
    @health_checkins = @health_checkins.reverse! if !@health_checkins.nil? && !@health_checkins.blank?
  end
